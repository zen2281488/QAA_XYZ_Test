...
import static testUtils.ConfProperties.getBoolProperty;
...
public class BrowserInit {
            ...
            switch (getProperty("env")) {
                case "remote":
                    switch (getProperty("browserName")) {
                        case "chrome":
                            webdriver.set(setUpRemoteDriver(new ChromeOptions()));
                            break;
                        default:
    ...
    private static RemoteWebDriver setUpRemoteDriver(MutableCapabilities options) {
        try {
            return new RemoteWebDriver(new URL(getProperty("hubUrl")), options);
